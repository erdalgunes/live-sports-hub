// GET /api/v1/matches/[id]/graph - Get match momentum graph
...
import { getMatchGraph, getMatchById } from '@/services/matches';
...
export async function GET(
  ...
  return withErrorHandling(async () => {
    ...
    // Fetch match graph data
    const graph = await getMatchGraph(matchId);

    if (!graph) {
      return apiNotFound('Match graph data');
    }
